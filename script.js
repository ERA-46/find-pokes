...
const answer2Or = pokes.filter(poke => poke.types.some(typ => types.includes(typ))).map(poke => poke.name);

const answer2And = pokes.filter(poke => poke.types.every(typ => types.includes(typ))).map(poke => poke.name);
